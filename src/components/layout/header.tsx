  ...
  return (
    <header className="bg-background/80 border-muted/10 sticky top-0 z-50 border-b backdrop-blur-md">
      <Container className="flex h-20 items-center justify-between gap-4">
        <Link
          href="/"
          className="text-sm font-medium tracking-tight lg:text-[20px] text-muted"
        >
        ...
        <nav aria-label="Main" className="flex items-center gap-4 sm:gap-6">
          {siteConfig.nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="text-muted hover:text-foreground text-xs transition-colors sm:text-sm lg:text-[20px]"
            >
              {item.label}
            </Link>
          ))}
